...
At [Axflow](https://axflow.dev), we are building [open source tooling](https://github.com/axflow/axflow) to help developers integrate LLM applications into their products.
...
We can add a function to ensure the request body's `stream` property is the expected value in all cases.
...
With that in place our module to handle both streaming and non-streaming calls to OpenAI's chat completion endpoint is in place! The final code is as follows:
...
Let's create an file at `lib/my_app_web/controllers/api/chat_controller.ex` with the following code.
...
Elixir is joy to work with. It took less than 70 lines of code to write a fully-functional chat completions wrapper and streaming API endpoint.
