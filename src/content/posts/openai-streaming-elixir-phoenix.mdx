...
published: 2023-01-05T00:55:43.715Z
---
...
The majority of this tooling thus far has been written in TypeScript, though we've been exploring replacing part of our stack with a Phoenix application.
...
If you follow along you'll need a Phoenix application and an OpenAI API key.
...
It's worth noting that some chunks in the streamed response contain multiple of these events, while others may only have one. We must keep this in mind when parsing.
...
The POST request body for this endpoint is expected to have a single key named `request` that contains any values supported by [OpenAI's chat completion request](https://platform.openai.com/docs/api-reference/chat/create).
...
_Note: Here we configure the :api pipeline only for JSON. By default it also fetches the session, which will fail if you want to try out the below curl request._
