...
This purpose of this post is to walk through implementing a module that can stream OpenAI chat completions. While we can use a third-party library,
we'll write this from scratch to learn how to parse streaming events over HTTP. We'll also learn how we can map over this stream and transform it
before sending it off to our own clients.
...
In the next post in this series, we'll dive deeper into parsing.
